...
type ServerState = { runId:string; phase:Phase; paused:boolean; cancelled:boolean; budgetCents:number; spentCents:number; remainingCents:number; rawSourceCount:number; familyCount:number; gap:{question:string; importance:string; state:string}; thesis:{open:string; afterNorthstar?:string; afterMeridian?:string; current:string}; claims:Claim[]; events:{id:string; type:string; label:string; at:string}[]; dossierReady:boolean; llm:{provider:string; status:string; model:string}; semanticStatus:string; runtime:RuntimeStatus; purchasePlan?:{sourceId:string; reason:string; gap:string; provider:'groq'|'fixture'; model:string; status:'LIVE'|'FALLBACK'}; config:ResearchConfig; sources:Source[] }
...
const publisherOptions = [
  { id:'financial-press', label:'Grid Operators Report', example:'synthetic fixture · grid reporting' },
  { id:'wire-services', label:'Northstar Wire', example:'synthetic fixture · supplier reporting' },
  { id:'public-data', label:'Public energy & buildout data', example:'synthetic fixture · open datasets' },
  { id:'specialist-research', label:'Technical Systems Review', example:'synthetic fixture · site constraints' },
  { id:'company-filings', label:'Vertex Compute filings', example:'synthetic fixture · company plans' },
  { id:'macro-research', label:'Buildout & market context', example:'synthetic fixtures · context only' },
  { id:'infrastructure-press', label:'GridScope Asia', example:'synthetic fixture · queue benchmarks' },
] as const
...
function AgentActionStep({ run }:{ run:ServerState }) {
  ...
  return <div className="agent-action-step"><div><span className="kicker">Agent action · {run.purchasePlan?.provider === 'groq' ? 'Groq' : 'fixture fallback'} · {runtime.label}</span><h2>{paid ? 'Bought the report that closes the grid gap.' : 'No paid source was selected.'}</h2><p>{source ? <><strong>{source.title}</strong> from {source.publisher}. {run.purchasePlan?.reason}</> : 'The retrieved sources did not include an affordable premium report.'}</p><dl className="action-facts"><div><dt>Cost</dt><dd>{source?.priceCents ? `${formatXrp((source.xrpDrops ?? 0) / 1_000_000)} · ≈ ${money(source.priceCents)}` : 'No purchase'}</dd></div><div><dt>Budget left</dt><dd>{formatXrp(centsToXrp(run.remainingCents))} · ≈ {money(run.remainingCents)}</dd></div><div><dt>Result</dt><dd>{paid ? (runtime.mode === 'live' ? 'Validated and unlocked' : 'Fixture simulation recorded · unlocked') : 'Not purchased'}</dd></div></dl><small>Groq compared the synthetic fixture metadata. The server enforced the budget and {runtime.mode === 'live' ? 'XRPL Testnet payment' : 'fixture simulation'} rules.</small></div>{source?.payment?.explorerUrl && <a className="primary-button" href={source.payment.explorerUrl} target="_blank" rel="noreferrer">View transaction <Icon name="arrow" /></a>}</div>
}
