...
function DossierPanel({ dossier, run, onOpenSource, onSynthesize, busy }:{ dossier:Dossier|null; run:ServerState; onOpenSource:(id:string)=>void; onSynthesize:()=>void; busy:boolean }) {
  ...
  return <section className="dossier-panel" id="dossier"><div className="dossier-heading"><div><span className="kicker">Verified dossier · {dossier.provider === 'groq' ? 'Groq' : 'fixture fallback'}</span><h2>{dossier.title}</h2></div><button type="button" className="small-button light" onClick={() => window.print()}>Print</button></div><div className="dossier-paper"><div className="dossier-kicker">{dossier.mode ?? 'FIXTURE RESEARCH'} · NOT INVESTMENT ADVICE</div><h3>{dossier.conclusion}</h3><div className="changed-callout"><span className="kicker">What changed after paid evidence</span><span className="change-line"><b>Open web</b>{dossier.changedAfterPaidResearch.before}</span>{dossier.changedAfterPaidResearch.afterNorthstar && <span className="change-line"><b>+ Northstar</b>{dossier.changedAfterPaidResearch.afterNorthstar}</span>}<span className="change-line final"><b>{dossier.afterLabel ?? '+ Grid report'}</b>{dossier.changedAfterPaidResearch.after}</span></div><div className="dossier-columns"><div><span className="kicker">Supports the thesis</span>{dossier.claims.filter((claim) => claim.stance === 'SUPPORTS').map((claim) => <ClaimBlock key={claim.id} claim={claim} sources={run.sources} onOpen={onOpenSource} />)}</div><div><span className="kicker">Challenges the thesis</span>{dossier.claims.filter((claim) => claim.stance !== 'SUPPORTS').map((claim) => <ClaimBlock key={claim.id} claim={claim} sources={run.sources} onOpen={onOpenSource} />)}</div></div><div className="dossier-footer"><div><span className="kicker">Key uncertainty</span><p>{dossier.uncertainty}</p></div><div><span className="kicker">Method & limitations</span><p>{dossier.method}</p></div></div></div></section>
}
...
function EvidenceStep({ run, sources, showAll, onShowAll, selectedId, onOpen, onAction }:{ run:ServerState; sources:Source[]; showAll:boolean; onShowAll:()=>void; selectedId:string|null; onOpen:(id:string)=>void; onAction:(sourceId:string, action:'BUY'|'SKIP'|'BLOCKED')=>void }) {
  ...
  return <><div className="step-copy"><div><span className="kicker">Retrieved evidence</span><h2>{sources.length} previews from approved websites</h2><p>Only these sources were searched. Match score ranks relevance; price is considered separately.</p></div><span className="step-summary mono">{run.familyCount} evidence families</span></div><div className="source-filter"><span>Websites:</span>{(run.config.sourceAllowlist ?? []).map((key) => <span key={key} className="filter-chip"><i />{publisherOptions.find((option) => option.id === key)?.label ?? key}</span>)}</div><div className="source-list">{visible.map((source) => <SourceItem key={source.id} source={source} selected={selectedId === source.id} onOpen={() => onOpen(source.id)} onAction={(action) => onAction(source.id, action)} />)}</div>{sources.length > visible.length && <button type="button" className="load-more" onClick={onShowAll}>Show {sources.length - visible.length} more sources <Icon name="arrow" /></button>}</>
}
...
function AgentActionStep({ run }:{ run:ServerState }) {
  ...
  return <div className="agent-action-step"><div><span className="kicker">Agent action · {run.purchasePlan?.provider === 'groq' ? 'Groq' : 'fixture fallback'}</span><h2>{paid ? 'Bought the report that closes the grid gap.' : 'No paid source was selected.'}</h2><p>{source ? <><strong>{source.title}</strong> from {source.publisher}. {run.purchasePlan?.reason}</> : 'The retrieved sources did not include an affordable premium report.'}</p><dl className="action-facts"><div><dt>Cost</dt><dd>{source?.priceCents ? `${formatXrp((source.xrpDrops ?? 0) / 1_000_000)} · ≈ ${money(source.priceCents)}` : 'No purchase'}</dd></div><div><dt>Budget left</dt><dd>{formatXrp(centsToXrp(run.remainingCents))} · ≈ {money(run.remainingCents)}</dd></div><div><dt>Result</dt><dd>{paid ? 'Paid and unlocked' : 'Not purchased'}</dd></div></dl><small>Groq compared the mock retrieval metadata. The server enforced the budget and payment rules.</small></div>{source?.payment?.explorerUrl && <a className="primary-button" href={source.payment.explorerUrl} target="_blank" rel="noreferrer">View transaction <Icon name="arrow" /></a>}</div>
}
...
function ResearchPath({ run, dossier, sources, showAll, onShowAll, selectedId, onOpenSource, onAction, onSynthesize, busy, synthesisText, synthesisStreaming }:{ run:ServerState; dossier:Dossier|null; sources:Source[]; showAll:boolean; onShowAll:()=>void; selectedId:string|null; onOpenSource:(id:string)=>void; onAction:(sourceId:string, action:'BUY'|'SKIP'|'BLOCKED')=>void; onSynthesize:()=>void; busy:boolean; synthesisText:string; synthesisStreaming:boolean }) {
  const hasPurchase = run.sources.some((source) => source.decision === 'BUY')
  return <section className="research-path" aria-label="Research path"><ol><li className="research-step"><span className="step-marker" aria-hidden="true">1</span><div className="step-content"><div className="step-heading"><span className="kicker">Search</span><span className="step-state">Complete</span></div><EvidenceStep run={run} sources={sources} showAll={showAll} onShowAll={onShowAll} selectedId={selectedId} onOpen={onOpenSource} onAction={onAction} /></div></li><li className="research-step"><span className="step-marker" aria-hidden="true">2</span><div className="step-content"><div className="step-heading"><span className="kicker">Purchase</span><span className={`step-state ${hasPurchase ? 'is-complete' : ''}`}>{hasPurchase ? 'Complete' : 'Waiting'}</span></div><AgentActionStep run={run} /></div></li><li className="research-step"><span className="step-marker" aria-hidden="true">3</span><div className="step-content"><div className="step-heading"><span className="kicker">Answer</span><span className={`step-state ${dossier ? 'is-complete' : ''}`}>{dossier ? 'Ready' : 'Next'}</span></div>{dossier ? <DossierPanel dossier={dossier} run={run} onOpenSource={onOpenSource} onSynthesize={onSynthesize} busy={busy} /> : <>{synthesisStreaming && <SynthesisStream text={synthesisText} streaming={synthesisStreaming} />}<WorkingAnswer run={run} onOpenSource={onOpenSource} /><div className="answer-action"><div><strong>Turn the unlocked evidence into a cited answer.</strong><span>Every claim will link to the exact excerpt used.</span></div><button type="button" className="primary-button" disabled={busy || !hasPurchase} onClick={onSynthesize}>Assemble cited answer <Icon name="arrow" /></button></div></>}</div></li></ol></section>
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    stream.addEventListener('DOSSIER_SYNTHESIS_FALLBACK', () => { setSynthesisStreaming(false); setMessage('Groq synthesis was unavailable; the cited fixture fallback was used.') })
  ...
  return <div className="app-shell">
    ...
    <footer className="statusbar"><span><span className="status-dot" /> {message}</span><span className="mono">{run ? `${run.events.length} events · ${run.llm.provider} · ${run.semanticStatus}` : 'Evidence first · citations stay traceable'}</span></footer>
